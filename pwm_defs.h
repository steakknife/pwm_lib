...
namespace arduino_due
{
    ...
    namespace pwm_lib
    {
        ...
        #define pin_traits_specialization(the_pwm_pin, pio, pio_pin, pio_id, pio_type, pio_conf, pwm_channel, pwm_inverted) \
            template<> struct pin_traits<the_pwm_pin> \
            { \
                static Pio* pio_p() { return pio; } \
        ...
        namespace pwm_core
        {
            ...
            inline double max_period(uint32_t clock) noexcept
            {
                return static_cast<double>(
                    (clock && clock <= max_clocks)
                        ? (static_cast<uint64_t>(1) << (two_power_values[clock] + 16)) / static_cast<double>(VARIANT_MCK)
                        : -1
                );
            }
