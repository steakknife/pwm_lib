...
namespace arduino_due
{
    namespace pwm_lib
    {
        template<pwm_pin PIN> class pwm
        {
        public:
            ...
            bool set_duty(uint32_t duty /* 1e-8 secs. */)
            {
                ...
                pwm_core::pwmc_setdutycycle(
                    PWM_INTERFACE,
                    pin_info::channel,
                    static_cast<uint32_t>((static_cast<double>(duty) / 100000000) / pwm_core::tick_times[_clock_])
                );
                return true;
            }
            ...
            )
            {
                ...
                if (keep_clock) {
                    ...
                    PWMC_SetPeriod(
                        PWM_INTERFACE,
                        pin_info::channel,
                        static_cast<uint32_t>(new_period / pwm_core::tick_times[_clock_])
                    );
                    set_duty(duty);
                } else {
                    uint32_t clock;
                    ...
                    if (clock == _clock_) {
                        ...
                        PWMC_SetPeriod(
                            PWM_INTERFACE,
                            pin_info::channel,
                            static_cast<uint32_t>((static_cast<double>(period) / 100000000) / pwm_core::tick_times[_clock_])
                        );
                        set_duty(duty);
                    } else {
                        _stop_();
        ...
        private:
            ...
            void _stop_()
            {
                ...
                while (PWM->PWM_SR & (1 << pin_info::channel)) ; // nothing

                _started_ = false;
            }
        }; // template<pwm_pin PIN> class pwm

        template<pwm_pin PIN> void pwm<PIN>::_start_(
            uint32_t period, // hundredths of usecs (1e-8 secs.)
            uint32_t duty, // hundredths of usecs (1e-8 secs.)
            uint32_t clock
        )
        {
            ...
            // setting PWM clocks
            PWM->PWM_CLK = 1 << (pwm_core::two_power_values[11] - pwm_core::two_power_values[10])         // clock A's DIVA value
                         | PWM_CMR_CPRE_MCK_DIV_1024 << 8                                                 // clock A's prescaler
                         | (1 << (pwm_core::two_power_values[12] - pwm_core::two_power_values[10])) << 16 // clock B's DIVB value
        ...
        public:
            ...
            virtual void stop() = 0;
            virtual uint32_t get_duty() const = 0;
            ...
            virtual uint32_t get_clock() const = 0;
        }; // class pwm_base

        template<typename PWM_TYPE> class pwm_wrapper: public pwm_base
        {
        public:
            pwm_wrapper(PWM_TYPE& pwm_obj)
                : _pwm_obj_(pwm_obj)
            {
            }
            ...
            uint32_t get_duty() const override
            {
            ...
            uint32_t get_clock() const override
            {
        ...
        template<pwm_pin PIN> class servo
        {
        public:
            using pin_info = pin_traits<PIN>;
            using pwm_t = pwm<PIN>;
            ...
            uint32_t get_angle() const {
                return _angle_;
            }

            bool set_angle(uint32_t angle /* degrees */)
            {
                if (!pwm_core::is_inside(_a_min_, _a_max_, angle)) return false;

                _angle_ = angle;

                uint32_t duty = (static_cast<double>(angle - _a_min_) / static_cast<double>(_a_max_ - _a_min_)) * (_t_max_ - _t_min_) + _t_min_;

                if (!_pwm_obj_.set_duty(duty)) return false;
            ...
            uint32_t get_t_min() const
            {
                return _t_min_;
            }
            uint32_t get_t_max() const
            {
                return _t_max_;
            }
            uint32_t get_a_min() const
            {
                return _a_min_;
            }
            uint32_t get_a_max() const
            {
                return _a_max_;
            }
        ...
        template<pwm_pin PIN> bool servo<PIN>::start(
            uint32_t period,    // hundredths of usecs (1e-8 secs)
            uint32_t time_min,  // hundredths of usecs (1e-8 secs)
            uint32_t time_max,  // hundredths of usecs (1e-8 secs)
            uint32_t angle_min, // degrees
        ...
        {
            if (angle_min > angle_max || time_min > time_max ||
            ...
            uint32_t duty = (static_cast<double>(duty_angle - angle_min) / static_cast<double>(angle_max - angle_min)) * (time_max - time_min) + time_min;

            if (!_pwm_obj_.start(period, duty)) return false;
        ...
        public:
            ...
            virtual void stop() = 0;
            virtual uint32_t get_angle() const = 0;
            virtual bool set_angle(uint32_t angle /* degrees */) = 0;
            virtual uint32_t get_period() const = 0;
            virtual uint32_t get_t_min() const = 0;
            virtual uint32_t get_t_max() const = 0;
            virtual uint32_t get_a_min() const = 0;
            virtual uint32_t get_a_max() const = 0;
        }; // class servo_base

        template<typename SERVO_TYPE> class servo_wrapper: public servo_base
        {
        public:
            servo_wrapper(SERVO_TYPE& servo_obj)
                : _servo_obj_(servo_obj)
            {
            }
            ...
            uint32_t get_angle() const override
            {
            ...
            uint32_t get_t_min() const override
            {
                return _servo_obj_.get_t_min();
            }

            uint32_t get_t_max() const override
            {
                return _servo_obj_.get_t_max();
            }

            uint32_t get_a_min() const override
            {
                return _servo_obj_.get_a_min();
            }

            uint32_t get_a_max() const override
            {
